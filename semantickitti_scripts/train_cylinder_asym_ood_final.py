...
def main(args):
    ...
    while epoch < train_hypers['max_num_epochs']:
        ...
        for i_iter, (_, train_vox_label, train_grid, _, train_pt_fea) in enumerate(train_dataset_loader):
            ...
            voxel_label_tensor[voxel_label_tensor == 5] = 0
            # 0~18
            voxel_label_tensor = torch.where(voxel_label_tensor <= 5, voxel_label_tensor, voxel_label_tensor - 1)
            ...
            voxel_label_obj[voxel_label_obj == 0] = -1
            voxel_label_obj[voxel_label_obj == 5] = 66
        ...
        with torch.no_grad():
            ...
            for i_iter_val, (_, val_vox_label, val_grid, val_pt_labs, val_pt_fea, idx) in enumerate(
                ...
                val_label_tensor[val_label_tensor == 5] = 0

                val_label_tensor = torch.where(val_label_tensor <= 5, val_label_tensor,
                                               val_label_tensor - 1)
                ...
                val_loss_center = loss_center_func(y_in_normal_valid, pt_label_origin.squeeze(), is_train=False)
                ...
                # 与原始标签对齐
                y_in_normal = np.where(y_in_normal < 4, y_in_normal + 1, y_in_normal + 2)
