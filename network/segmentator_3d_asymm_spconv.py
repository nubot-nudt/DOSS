...
class ResContextBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        x.features = self.act2(x.features)
        x.features = self.bn1(x.features)

        x = self.conv3(x)
        x.features = self.act3(x.features)
        x.features = self.bn2(x.features)
        x.features = x.features + shortcut.features
...
class UpBlock(nn.Module):
    ...
    def forward(self, x, skip):
        x = self.trans_dilao(x)
        x.features = self.trans_act(x.features)
        x.features = self.trans_bn(x.features)
        ...
        x = self.conv1(x)
        x.features = self.act1(x.features)
        x.features = self.bn1(x.features)
        ...
        x = self.conv3(x)
        x.features = self.act3(x.features)
        x.features = self.bn3(x.features)
...
class Decoder(nn.Module):
    def __init__(self, init_size=16, nclasses=18, indice_key=None):
        ...
        self.logits = spconv.SubMConv3d(4 * init_size, nclasses, indice_key=indice_key+"logitD", kernel_size=3, stride=1, padding=1,
                                        bias=True)
    ...
    def forward(self, x):
        ...
        logits_normal = self.logits(up0e)
        logits_normal = logits_normal.dense()
...
class Asymm_3d_spconv(nn.Module):
    ...
    def forward_ow(self, voxel_features, coors, batch_size):
        ...
        y_in_normal = self.decoder_cw(x=outs)
        y_out_normal = self.decoder_ow(x=outs)

        return coor_ori, y_in_normal, y_out_normal
